										...
										<table class="table table-bordered" id="example1" style="width:100%;">
											...
											<tbody>
												...
												<form method="POST" action="fungsi/edit/edit.php?jual=jual">
														<input type="number" min="0" value="1" name="jumlah" value="<?php echo $isi['jumlah'];?>" class="form-control">
														<input type="hidden" name="id" value="<?php echo $isi['id_penjualan'];?>" class="form-control">
														<input type="hidden" name="id_barang" value="<?php echo $isi['id_barang'];?>" class="form-control">
													</td>
													<td>Rp.<?php echo number_format($isi['total']);?>,-</td>
													<td><?php echo $isi['nm_member'];?></td>
													<td><?php echo $isi['pelanggans'];?></td>
											...
											if(!empty($_GET['nota'] == 'yes')) {
												...
												if(!empty($bayar))
												{
													...
													if($bayar >= $total)
													{
														...
														//jan lupa nambahin buat field diskon sama taotal akhir
														for($x=0;$x<$jumlah_dipilih;$x++){

															$d = array($id_barang[$x],$id_member[$x],$jumlah[$x],$total[$x],$tgl_input[$x],$bulan[$x],$tahun[$x],$periode[$x]);
															$sql = "INSERT INTO nota (id_barang,id_member,jumlah,total,tanggal_input,bulan,tahun,periode) VALUES(?,?,?,?,?,?,?,?)";
...
// 		$.ajax({
// 		type: "POST",
// 		url: "fungsi/edit/edit.php?potongan-diskon=yes",
// 		data:'keyword='+$(this).val(),
// 		beforeSend: function(){
//             $("#hasil_dis").hide();
// 			$("#tunggu").html('<p style="color:green"><blink>tunggu sebentar</blink></p>');

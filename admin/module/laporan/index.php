                  ...
                  <div class="col-lg-12 main-chart">
						<h3>
							...
							<?php if(!empty($_GET['tahun'])){ ?>
								Data Laporan Penjualan Tahun <?= $_POST['thn'];?>
							<?php }elseif(!empty($_GET['bulan'])){?>
								Data Laporan Penjualan <?= $bulan_tes[$_POST['bln']];?> - <?= $bulan_tes[$_POST['buln']];?>
						...
						<br/>
						<h4>Cari Laporan Bulanan</h4>
						<form method="post" action="index.php?page=laporan&bulan=ok">
							<table class="table table-striped">
								...
								<td>
								<select name="buln" class="form-control">
									<option selected="selected">Bulan</option>
									<?php
										$bulan=array("Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember");
										$jlh_bln=count($bulan);
								...
								<td>
									<input type="hidden" name="periodebulan" value="ya">
									...
									<?php if(!empty($_GET['bulan'])){?>
										<a href="excel.php?bulan=yes&bln=<?=$_POST['bln'];?>&buln=<?=$_POST['buln'];?>" class="btn btn-info"><i class="fa fa-download"></i>
										Excel</a>
									<?php }else{?>
						...
						<h4>Cari Laporan Per Tahun</h4>
						<form method="post" action="index.php?page=laporan&tahun=ok">
							<table class="table table-striped">
								<tr>
									<!-- <th>
										Pilih Bulan
								...
								<td>
									<input type="hidden" name="periodetahun" value="ya">
									<button class="btn btn-primary">
										<i class="fa fa-search"></i> Cari
									</button>
									<a href="index.php?page=laporan" class="btn btn-success">
									...
									<?php if(!empty($_GET['tahun'])){?>
										<a href="excel.php?tahun=yes&thn=<?=$_POST['thn'];?>" class="btn btn-info"><i class="fa fa-download"></i>
										Excel</a>
									<?php }else{?>
										<a href="excel.php" class="btn btn-info"><i class="fa fa-download"></i>
										Excel</a>
							...
							<table class="table table-bordered" id="example1">
								...
								<tbody>
									<?php 
										$no=1; 
										if(!empty($_GET['tahun'])){
											$periodetahun = $_POST['thn'];
											// $periode = $_POST['bln'].'-'.$_POST['thn'];
											$no=1; 
											$jumlah = 0;
											$bayar = 0;
											$hasil = $lihat -> tahun_jual($periodetahun);
										}elseif(!empty($_GET['bulan'])){
											$periodebulan = $_POST['bln'];
											$periodeBulans = $_POST['buln'];
											$no=1; 
											$jumlah = 0;
											$bayar = 0;
											$hasil = $lihat -> bulan_jual($periodebulan, $periodeBulans);

...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['customer'])){
		...
		$data[] = $alamat;
		$data[] = $id;
		$sql = 'UPDATE customer SET  nama_pelanggan=?, alamat_pelanggan=? WHERE id_pelanggan=?';
		$row = $config -> prepare($sql);
		$row -> execute($data);
	...
	if(!empty($_GET['diskon'])){
		$id= htmlentities($_POST['id']);
		$pelanggan= htmlentities($_POST['customer_id']);
		$persen= htmlentities($_POST['persen']);
		$tgl = htmlentities($_POST['tgl_update']);
		$data[] = $pelanggan;
		$data[] = $persen;
		$data[] = $tgl;
		$data[] = $id;
		$sql = 'UPDATE diskon SET  customer_id=?, persen=?, tgl_update=?  WHERE id_diskon=?';
		$row = $config -> prepare($sql);
		$row -> execute($data);
		// var_dump($data);
		echo '<script>window.location="../../index.php?page=diskon&uid='.$id.'&success-edit=edit-data"</script>';
	}

...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['customer'])){
		$id= $_GET['id'];
		$data[] = $id;
		$sql = 'DELETE FROM customer WHERE id_pelanggan=?';
		$row = $config -> prepare($sql);
		$row -> execute($data);
	...
	if(!empty($_GET['diskon'])){
		$id= $_GET['id'];
		$data[] = $id;
		$sql = 'DELETE FROM diskon WHERE id_diskon=?';
		$row = $config -> prepare($sql);
		$row -> execute($data);
		echo '<script>window.location="../../index.php?page=diskon&&remove=hapus-data"</script>';
	}
